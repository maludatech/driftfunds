...
const Deposit = ({ callbackUrl = "/sign-in" }: { callbackUrl?: string }) => {
  const { user } = useAuthStore();
  const { selectedPlan, selectedCoin, setPlan, setTotalAmount } =
    usePlanStore();
  ...
  const [localSelectedCoin, setLocalSelectedCoin] = useState<string | null>(
    null
  );
  ...
  const handleProceedToPay = () => {
    ...
    if (
      !enteredAmount ||
      amount < localSelectedPlan.minInvestment ||
      amount > localSelectedPlan.maxInvestment
    ) {
      toast.error(
        `Please enter an amount between $${localSelectedPlan.minInvestment} and $${localSelectedPlan.maxInvestment} without commas.`
      );
      setIsLoading(false);
      return;
    }
    ...
    try {
      setPlan({ plan: localSelectedPlan, coin: localSelectedCoin });
      setTotalAmount(enteredAmount);
      router.push(`/deposit/new/${localSelectedPlan.name.toLowerCase()}`);
    } catch (error) {
      log.error("Error proceeding to payment:", error);
      ...
      <div className="body-container flex flex-col sm:flex-row gap-2 w-full">
        ...
        <div className="lg:ml-[200px] 2xl:ml-[150px] flex flex-col gap-4 p-1 w-full">
          ...
          <p className="text-sm text-muted-foreground">
            Select an investment plan and cryptocurrency to proceed with your
            deposit.
          </p>
          ...
          <div className="rounded-lg bg-card/80 p-6 shadow-lg backdrop-blur-sm dark:shadow-gray-800">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {Plans.map((plan, index) => (
                <div
                  ...
                  className={`rounded-lg bg-card/80 p-4 shadow-md hover:shadow-lg transition-shadow hover:cursor-pointer border-2 ${
                    localSelectedPlan?.name === plan.name
                      ? "border-primary"
                ...
                >
                  ...
                  <p className="text-center text-lg">
                    <span className="text-xl">${plan.deposit}</span> /{" "}
                    {plan.totalReturn}% Profit
                  </p>
                  <ul className="mt-2 text-sm text-center text-muted-foreground">
                ...
                return (
                  <div key={idx} className="flex items-center mb-2">
                    <input
                      ...
                      checked={localSelectedCoin === coin.name}
                      onChange={() => setLocalSelectedCoin(coin.name)}
                    />
                    <label
            ...
            {/* Amount Input and Proceed Button */}
            <Separator className="my-6 bg-gray-200 dark:bg-gray-700" />
